<?php

declare(strict_types=1);

namespace PhpModern\Kernel\Tests\Console;

use PhpModern\Console\BufferedOutput;
use PhpModern\Console\Input;
use PhpModern\Kernel\Console\MigrateConsoleCommand;
use PHPUnit\Framework\TestCase;

final class MigrateConsoleCommandTest extends TestCase
{
    private string $tmpDir;
    private string $originalCwd;

    protected function setUp(): void
    {
        $this->tmpDir = sys_get_temp_dir() . '/phpmodern-migrate-console-test-' . uniqid();
        mkdir($this->tmpDir . '/migrations', 0777, true);
        $this->originalCwd = (string) getcwd();
        chdir($this->tmpDir);
    }

    protected function tearDown(): void
    {
        chdir($this->originalCwd);
        @unlink($this->tmpDir . '/test.sqlite');

        foreach (glob($this->tmpDir . '/migrations/*.php') ?: [] as $file) {
            unlink($file);
        }

        rmdir($this->tmpDir . '/migrations');
        rmdir($this->tmpDir);
    }

    public function test_missing_dsn_fails_with_a_clear_message(): void
    {
        $output = new BufferedOutput();
        $exitCode = (new MigrateConsoleCommand())->handle(new Input(['--dir=migrations']), $output);

        self::assertSame(1, $exitCode);
        self::assertStringContainsString('Missing database DSN', $output->lines()[0]);
    }

    public function test_runs_pending_migrations_against_the_given_dsn(): void
    {
        file_put_contents($this->tmpDir . '/migrations/20260101000000_create_widgets.php', <<<'PHP'
            <?php
            use PhpModern\Orm\Connection;
            use PhpModern\Orm\Migration;
            return new class implements Migration {
                public function up(Connection $connection): void {
                    $connection->pdo()->exec('CREATE TABLE widgets (id INTEGER PRIMARY KEY)');
                }
                public function down(Connection $connection): void {
                    $connection->pdo()->exec('DROP TABLE widgets');
                }
            };
            PHP);

        $output = new BufferedOutput();
        $exitCode = (new MigrateConsoleCommand())->handle(
            new Input(['--dsn=sqlite:test.sqlite', '--dir=migrations']),
            $output,
        );

        self::assertSame(0, $exitCode);
        self::assertSame(['Migrated: 20260101000000_create_widgets'], $output->lines());

        $second = new BufferedOutput();
        (new MigrateConsoleCommand())->handle(
            new Input(['--dsn=sqlite:test.sqlite', '--dir=migrations']),
            $second,
        );
        self::assertSame(['Nothing to migrate.'], $second->lines());
    }
}
